...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = com.neon.android.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.neon.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = com.neon.android.model.PrivateMessagePacket.decode(noisePayload.data)
                ...
                com.neon.android.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.neon.android.model.BitchatFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "🔓 Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.neon.android.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.neon.android.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                com.neon.android.model.NoisePayloadType.DELIVERED -> {
                ...
                com.neon.android.model.NoisePayloadType.READ_RECEIPT -> {
                ...
                com.neon.android.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                ...
                com.neon.android.model.NoisePayloadType.VERIFY_RESPONSE -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.neon.android.model.NoisePayload(
                type = com.neon.android.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (age > com.neon.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
            Log.w(TAG, "Ignoring stale ANNOUNCE from ${peerID.take(8)} (age=${age}ms > ${com.neon.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
            return false
        }
        ...
        try {
            val neighborsOrNull = com.neon.android.services.meshgraph.GossipTLV.decodeNeighborsFromAnnouncementPayload(packet.payload)
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = BitchatPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = com.neon.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = com.neon.android.protocol.MessageType.fromValue(packet.type) == com.neon.android.protocol.MessageType.FILE_TRANSFER
            val file = com.neon.android.model.BitchatFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.neon.android.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    com.neon.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    com.neon.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }
                ...
                val sys = com.neon.android.model.BitchatMessage(
                    sender = "system",
