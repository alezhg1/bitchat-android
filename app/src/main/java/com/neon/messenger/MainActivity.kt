...
import com.neon.messenger.onboarding.BluetoothCheckScreen
import com.neon.messenger.onboarding.BluetoothStatus
import com.neon.messenger.onboarding.BluetoothStatusManager
import com.neon.messenger.onboarding.BatteryOptimizationManager
import com.neon.messenger.onboarding.BatteryOptimizationPreferenceManager
import com.neon.messenger.onboarding.BatteryOptimizationScreen
import com.neon.messenger.onboarding.BatteryOptimizationStatus
import com.neon.messenger.onboarding.BackgroundLocationPermissionScreen
import com.neon.messenger.onboarding.InitializationErrorScreen
import com.neon.messenger.onboarding.InitializingScreen
import com.neon.messenger.onboarding.LocationCheckScreen
import com.neon.messenger.onboarding.LocationStatus
import com.neon.messenger.onboarding.LocationStatusManager
import com.neon.messenger.onboarding.OnboardingCoordinator
import com.neon.messenger.onboarding.OnboardingState
import com.neon.messenger.onboarding.PermissionExplanationScreen
import com.neon.messenger.onboarding.PermissionManager
import com.neon.messenger.ui.ChatScreen
import com.neon.messenger.ui.ChatViewModel
import com.neon.messenger.ui.OrientationAwareActivity
import com.neon.messenger.ui.theme.BitchatTheme
import com.neon.messenger.nostr.PoWPreferenceManager
...
class MainActivity : OrientationAwareActivity() {
    ...
    private val forceFinishReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context, intent: android.content.Intent) {
            if (intent.action == com.neon.messenger.util.AppConstants.UI.ACTION_FORCE_FINISH) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(
                forceFinishReceiver,
                filter,
                com.neon.messenger.util.AppConstants.UI.PERMISSION_FORCE_FINISH,
                null,
        ...
        com.neon.messenger.service.AppShutdownCoordinator.cancelPendingShutdown()
        ...
        try { com.neon.messenger.service.MeshForegroundService.start(applicationContext) } catch (_: Exception) { }
        meshService = com.neon.messenger.service.MeshServiceHolder.getOrCreate(applicationContext)
            ...
            if (permissionManager.isFirstTimeLaunch()) {
                ...
                if (permissionManager.needsBackgroundLocationPermission() &&
                    !permissionManager.isBackgroundLocationGranted() &&
                     com.neon.messenger.onboarding.BackgroundLocationPreferenceManager.isSkipped(this@MainActivity)
                ) {
                    mainViewModel.updateOnboardingState(OnboardingState.BACKGROUND_LOCATION_EXPLANATION)
                } else {
    ...
    private fun initializeApp() {
        ...
        lifecycleScope.launch {
            try {
                ...
                com.neon.messenger.nostr.LocationNotesInitializer.initialize(this@MainActivity)
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            com.neon.messenger.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        val shouldOpenGeohashChat = intent.getBooleanExtra(
            com.neon.messenger.ui.NotificationManager.EXTRA_OPEN_GEOHASH_CHAT,
            false
        )
            ...
            shouldOpenGeohashChat -> {
                ...
                if (geohash != null) {
                    ...
                    val level = when (geohash.length) {
                        7 -> com.neon.messenger.geohash.GeohashChannelLevel.BLOCK
                        6 -> com.neon.messenger.geohash.GeohashChannelLevel.NEIGHBORHOOD
                        5 -> com.neon.messenger.geohash.GeohashChannelLevel.CITY
                        4 -> com.neon.messenger.geohash.GeohashChannelLevel.PROVINCE
                        2 -> com.neon.messenger.geohash.GeohashChannelLevel.REGION
                        else -> com.neon.messenger.geohash.GeohashChannelLevel.CITY // Default fallback
                    }
                    val geohashChannel = com.neon.messenger.geohash.GeohashChannel(level, geohash)
                    val channelId = com.neon.messenger.geohash.ChannelID.Location(geohashChannel)
    ...
    private fun handleVerificationIntent(intent: Intent) {
        ...
        if (uri.scheme != "neon" || uri.host != "verify") return
