package com.neon.messenger.mesh

import android.content.Context
import android.util.Log
import com.neon.messenger.crypto.EncryptionService
import com.neon.messenger.model.BitchatMessage
import com.neon.messenger.protocol.MessagePadding
import com.neon.messenger.model.RoutedPacket
import com.neon.messenger.model.IdentityAnnouncement
import com.neon.messenger.model.NoisePayload
import com.neon.messenger.model.NoisePayloadType
import com.neon.messenger.protocol.BitchatPacket
import com.neon.messenger.protocol.MessageType
import com.neon.messenger.protocol.SpecialRecipients
import com.neon.messenger.model.RequestSyncPacket
import com.neon.messenger.sync.GossipSyncManager
import com.neon.messenger.util.toHexString
import com.neon.messenger.services.VerificationService
...
class BluetoothMeshService(private val context: Context) {
    private val debugManager by lazy { try { com.neon.messenger.ui.debug.DebugSettingsManager.getInstance() } catch (e: Exception) { null } }
    
    companion object {
        private const val TAG = "BluetoothMeshService"
        private val MAX_TTL: UByte = com.neon.messenger.util.AppConstants.MESSAGE_TTL_HOPS
    }
    ...
    private val serviceNotificationManager = com.neon.messenger.ui.NotificationManager(
        context.applicationContext,
        androidx.core.app.NotificationManagerCompat.from(context.applicationContext),
        com.neon.messenger.util.NotificationIntervalManager()
    )
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.neon.messenger.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.neon.messenger.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.neon.messenger.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { com.neon.messenger.services.AppStateStore.setPeers(peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { com.neon.messenger.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.neon.messenger.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.neon.messenger.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
                ...
                try {
                    when {
                        message.isPrivate -> {
                            ...
                            if (peer.isNotEmpty()) com.neon.messenger.services.AppStateStore.addPrivateMessage(peer, message)
                        }
                        message.channel != null -> {
                            com.neon.messenger.services.AppStateStore.addChannelMessage(message.channel!!, message)
                        }
                        else -> {
                            com.neon.messenger.services.AppStateStore.addPublicMessage(message)
                        }
                    }
                } catch (_: Exception) { }
                ...
                if (delegate == null && message.isPrivate) {
                    try {
                        ...
                        if (senderPeerID != null) {
                            ...
                            val preview = com.neon.messenger.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
            ...
            override fun handleAnnounce(routed: RoutedPacket) {
                serviceScope.launch {
                    ...
                    if (deviceAddress != null && pid != null) {
                        ...
                        val isDirect = routed.packet.ttl == com.neon.messenger.util.AppConstants.MESSAGE_TTL_HOPS
        ...
        override fun onPacketReceived(packet: BitchatPacket, peerID: String, device: android.bluetooth.BluetoothDevice?) {
            ...
            try {
                com.neon.messenger.ui.debug.DebugSettingsManager.getInstance().logIncoming(
                    packet = packet,
                    fromPeerID = peerID,
                    fromNickname = null,
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.neon.messenger.ui.debug.DebugSettingsManager.getInstance()
     ...
     */
    fun sendFileBroadcast(file: com.neon.messenger.model.BitchatFilePacket) {
        try {
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: com.neon.messenger.model.BitchatFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = com.neon.messenger.model.NoisePayload(
                            type = com.neon.messenger.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
                        ...
                        val packet = BitchatPacket(
                            ...
                            payload = encrypted,
                            signature = null,
                            ttl = com.neon.messenger.util.AppConstants.MESSAGE_TTL_HOPS
                        )
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.neon.messenger.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.neon.messenger.model.NoisePayload(
                        type = com.neon.messenger.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.neon.messenger.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.neon.messenger.nostr.GeohashAliasRegistry.snapshot()
            ...
            try {
                ...
                val seenStore = try { com.neon.messenger.services.SeenMessageStore.getInstance(context.applicationContext) } catch (_: Exception) { null }
                ...
                val readReceiptPayload = com.neon.messenger.model.NoisePayload(
                    type = com.neon.messenger.model.NoisePayloadType.READ_RECEIPT,
                ...
                val packet = BitchatPacket(
                    ...
                    payload = encrypted,
                    signature = null,
                    ttl = com.neon.messenger.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS messageTTL
                )
    ...
    fun sendBroadcastAnnounce() {
        ...
        serviceScope.launch {
            val nickname = try { com.neon.messenger.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
            ...
            try {
                ...
                if (directPeers.isNotEmpty()) {
                    val gossip = com.neon.messenger.services.meshgraph.GossipTLV.encodeNeighbors(directPeers)
                    tlvPayload = tlvPayload + gossip
                }
                // Always update our own node in the mesh graph with the neighbor list we used
                try {
                    com.neon.messenger.services.meshgraph.MeshGraphService.getInstance()
     ...
     */
    fun getSessionState(peerID: String): com.neon.messenger.noise.NoiseSession.NoiseSessionState {
        ...
        return try {
            ...
            val withRoute = try {
                ...
                if (rec != null && !rec.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.neon.messenger.services.meshgraph.RoutePlanner.shortestPath(myPeerID, dest)
