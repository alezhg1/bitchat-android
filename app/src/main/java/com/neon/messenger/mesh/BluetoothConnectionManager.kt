...
class BluetoothConnectionManager(
    ...
    init {
        ...
        try {
            val dbg = com.neon.messenger.ui.debug.DebugSettingsManager.getInstance()
            // Role enable/disable
    ...
    fun startServices(): Boolean {
        ...
        // }
            ...
            connectionScope.launch {
                ...
                val dbg = try { com.neon.messenger.ui.debug.DebugSettingsManager.getInstance() } catch (_: Exception) { null }
    ...
    override fun onPowerModeChanged(newMode: PowerManager.PowerMode) {
        ...
        connectionScope.launch {
            ...
            val serverEnabled = try { com.neon.messenger.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true }
            if (serverEnabled) {
            ...
            if (wasUsingDutyCycle != nowUsingDutyCycle) {
                ...
                val clientEnabled = try { com.neon.messenger.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
                if (clientEnabled) {
