package com.neon.messenger.nostr

import android.app.Application
import android.util.Log
import com.neon.messenger.model.BitchatFilePacket
...
import com.neon.messenger.model.PrivateMessagePacket
import com.neon.messenger.protocol.BitchatPacket
import com.neon.messenger.services.SeenMessageStore
import com.neon.messenger.ui.ChatState
import com.neon.messenger.ui.MeshDelegateHandler
import com.neon.messenger.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.neon.messenger.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (packet.type != com.neon.messenger.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.neon.messenger.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                if (file != null) {
                    ...
                    val savedPath = com.neon.messenger.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
