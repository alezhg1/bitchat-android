...
|--------|------|-----------|
| **1.4.0** | 2026-07-08 | Пользовательские группы с QR; офлайн-карта местности OpenTopoMap (OSMDroid) |
...
| **Группы (чаты)** | Любой создаёт чат → QR → участники сканируют; сообщения по mesh multi-hop |
| **Карта** | OpenTopoMap офлайн-кэш + маркеры участников |
...
**OSMDroid + OpenTopoMap** — рельеф, леса, тропы. Тайлы кэшируются в `filesDir/osmdroid` при первом просмотре **с интернетом**, дальше работают офлайн.

| Функция | Описание |
|---------|----------|
| **Базовая карта** | OpenTopoMap (местность, ориентиры) |
| **Маркеры** | Участники с GPS по mesh GEOLOC |
| **Список** | Все пользователи под картой |
| **Офлайн** | Кэш тайлов + mesh-позиции без сети |

Классы: `CampOfflineMapProvider`, `CampTerrainMapView`, `MapScreen`.
...
*Документ v1.4.0 — ветка `feat/messenger-roles-geolocation`. При мерже в main обновите версию и changelog.*
