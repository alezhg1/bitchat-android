...
**Версия приложения:** 1.7.2 (versionCode 33)  
**Дата:** 2026-07-08
...
|--------|------|-----------|
| **1.3.0** | 2026-07-08 | Роли через QR: админ сканирует QR оператора; преподаватель — только QR от админа в настройках |
| **1.2.0** | 2026-07-08 | Карта: mesh-топология, pinch-zoom, полный список пользователей; GEOLOC каждые **5 мин**; multi-hop через BLE |
| **1.1.0** | 2026-07-08 | Camp chat (единый чат лагеря), hybrid mesh + Nostr, `CampChatManager`, TECHDOC создан |
| **1.0.0** | — | Базовые доработки: роли, UI мессенджера, персистентность, onboarding fixes |
...
| **Карта (админ/учитель)** | Офлайн-карта с mesh-сетью, pinch-zoom, список всех пользователей; GEOLOC каждые 5 мин |
| **Роли** | Ученик — без QR. Админ — сканирует QR оператора. Преподаватель — только QR от админа (Настройки → «QR преподавателя»). |
...
## 4. Роли и аутентификация (QR, офлайн)

### 4.1. Роли

| Роль | Как получить | Возможности |
|------|--------------|-------------|
| `STUDENT` | Выбор при входе | Чаты, mesh, geo-пакеты |
| `TEACHER` | **Скан QR от администратора** | + офлайн-карта |
| `ADMIN` | **Скан QR оператора лагеря** | + карта, выдача QR преподавателя |

Пароли вручную **не вводятся** — только локальное сканирование камерой.

### 4.2. Формат QR

```
nlogn://role/v1/ADMIN#<url-encoded-key>
nlogn://role/v1/TEACHER#<url-encoded-key>
```

- Сканируется **локально**, не передаётся по mesh/Nostr.
- `RoleQrCodec` — parse/encode.
- `RoleKeyManager.grantFromQrPayload()` — проверка SHA-256 + сохранение grant.

### 4.3. Поток выдачи ролей

```
Оператор лагеря                    Администратор                 Преподаватель
     │                                  │                              │
     │  QR админа (распечатка)          │                              │
     ├─────────────────────────────────►│ сканирует при входе          │
     │                                  │                              │
     │                                  │  Настройки → QR преподавателя│
     │                                  ├─────────────────────────────►│ сканирует
     │                                  │  (показывает экран)          │ при входе
```
...
|-----------|------------|
| `RoleQrSheet.kt` | Сканер и отображение QR (CameraX + ML Kit) |
| `LoginScreen` | Кнопка «Сканировать QR» для admin/teacher |
| `AboutSheet` | Блок «QR преподавателя» (только `UserRole.ADMIN`) |
| `RoleQrRepository` | Загрузка payload из `role_qr.enc` |
| `RoleKeyManager` | Хеши ключей, verify grant |
| `tools/generate_role_keys.ps1` | Генерация `role_keys.enc` + `role_qr.enc` |
| `tools/GenerateRoleAssets.java` | Альтернатива без PowerShell 7 |

Plaintext ключи **не хранятся в коде** — только в зашифрованных assets, собранных оператором.

### 4.5. Выход из аккаунта
...
### 5.1. Протокол GEOLOC (mesh, офлайн)

Каждые **5 минут** (`LocationSharingService.SHARE_INTERVAL_MS`) устройство рассылает **broadcast** по BLE mesh:

```
[GEOLOC]:{staticId}|{fio}|{lat}|{lon}|{ROLE}|{timestampMs}
```

**Multi-hop (через участников):** пакет имеет `ttl = MAX_TTL` и ретранслируется промежуточными узлами через `PacketRelayManager` — данные доходят до админа/учителя даже если отправитель не в прямом Bluetooth-радиусе. Интернет **не требуется**.

Сообщения `[GEOLOC]` **не попадают** в ленту чата (`MeshDelegateHandler` фильтрует их), но каждый узел mesh может ретранслировать пакет дальше.

### 5.2. LocationSharingService

- GPS + кэш последней позиции + JSON-кэш на диске (`user_locations_cache.json`).
- Привязка к `BluetoothMeshService` и scope ViewModel.
- Старт в `onAppReady()`, стоп при `logout()`.
- `requestImmediateBroadcast()` — принудительная отправка (кнопка «Обновить» на карте).

### 5.3. MapScreen (админ / учитель)

Офлайн Canvas-карта без тайлов OSM:

| Функция | Описание |
|---------|----------|
| **Mesh-топология** | Линии между участниками: сплошные — прямые BLE-соседи; пунктир — получено через multi-hop |
| **Масштаб** | Pinch-zoom, кнопки +/-, «вписать всё» |
| **Якорь лагеря** | Центр из `camp_config.json` или geohash (`CampChatManager.getCampAnchorCoordinates`) |
| **Список пользователей** | Под картой: все известные участники (GPS, роль, mesh-статус, время обновления) |
| **Статистика** | Всего / с GPS / mesh рядом / интервал обновления |

Доступ: только `UserRole.TEACHER` и `UserRole.ADMIN` (кнопка карты в шапке).
...
*Документ v1.3.0 — ветка `feat/messenger-roles-geolocation`. При мерже в main обновите версию и changelog.*
